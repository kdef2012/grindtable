...
export function getKbjWinstonSalemLayout(): TableElement[] {
  ...
  [11, 12, 13, 14, 15, 16, 17].forEach((num, index) => {
    add({ number: num.toString(), type: '4_booth', position: { x: col1X, y: 100 + index * 120 } });
  });

  // COLUMN 2: 4-Tops
  const col2X = 250;
  [21, 22, 23, 24, 25, 26, 27].forEach((num, index) => {
    add({ number: num.toString(), type: '4_top', shape: 'square', position: { x: col2X, y: 100 + index * 120 } });
  });

  // COLUMN 3 & 4: Back-to-Back Booths
  const col3X = 400;
  const col4X = 500;
  [31, 32, 33, 34, 35].forEach((num, index) => {
    add({ number: num.toString(), type: '4_booth', position: { x: col3X, y: 100 + index * 120 } });
    add({ number: (num + 10).toString(), type: '4_booth', position: { x: col4X, y: 100 + index * 120 } });
  });
  ...
  // Bar Stools (Left: 8, Corners: 2 each, Front: 6, Right: 8)
  ...
  // ROW 6: Center Bottom 4-Tops
  [61, 62, 63, 64].forEach((num, index) => {
    add({ number: num.toString(), type: '4_top', shape: 'square', position: { x: 650 + index * 120, y: 700 } });
  });

  // ROW 7: Bottom Right
  [71, 72].forEach((num, index) => {
    add({ number: num.toString(), type: '4_top', shape: 'square', position: { x: 900 + index * 120, y: 820 } });
  });
  // 73 (Large Party of 10)
  add({ number: '73', type: '8_top', shape: 'rectangle', capacity: 10, position: { x: 1150, y: 820 }, width: 180, height: 80 });

  // COLUMN 6: Right Side Tables
  const col6X = 1300;
  [81, 82, 83, 84].forEach((num, index) => {
    add({ number: num.toString(), type: '4_top', shape: 'square', position: { x: col6X, y: 100 + index * 120 } });
  });

  // COLUMN 7: Right Wall Booths
  const col7X = 1450;
  [91, 92, 93, 94, 95, 96].forEach((num, index) => {
    // 95 and 96 are shifted down
    const yPos = index >= 4 ? 100 + (index + 0.5) * 120 : 100 + index * 120;
    add({ number: num.toString(), type: '4_booth', position: { x: col7X, y: yPos } });
  });
